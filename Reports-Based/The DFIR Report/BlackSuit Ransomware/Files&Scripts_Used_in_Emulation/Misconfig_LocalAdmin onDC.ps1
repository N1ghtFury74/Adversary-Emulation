...
foreach ($serviceAccount in $serviceAccountsForSPNs) {
    ...
    if ($account) {
        ...
        # Add svc-fileshare to Local Administrators group on the Domain Controller
        if ($serviceAccount.Name -eq "svc-fileshare") {
            try {
                # Get NetBIOS domain name (e.g., "CORP" for corp.local)
                $domainNetBios = (Get-ADDomain).NetBIOSName
                $domainQualifiedAccount = "$domainNetBios\$($serviceAccount.Name)"

                # Get the local Administrators group
                $localAdminsGroup = [ADSI]"WinNT://$env:COMPUTERNAME/Administrators,group"
                $currentMembers = $localAdminsGroup.psbase.Invoke("Members") | 
                    ForEach-Object { $_.GetType().InvokeMember("Name", 'GetProperty', $null, $_, $null) }

                # Check membership using NetBIOS domain format
                if ($currentMembers -contains $domainQualifiedAccount) {
                    Write-Output "Account $domainQualifiedAccount is already a member of the Local Administrators group."
                } else {
                    # Add using domain-qualified account name
                    $localAdminsGroup.psbase.Invoke("Add", "WinNT://$domainNetBios/$($serviceAccount.Name),user")
                    Write-Output "Account $domainQualifiedAccount added to Local Administrators on the Domain Controller."
                }
            } catch {
                Write-Output "Failed to add $domainQualifiedAccount to Local Administrators: $_"
            }
        }
    } else {
